...
public sealed class InstanceInterceptorActionT3<TTarget, T1, T2, T3>
{
    ...
    readonly Action<TTarget, T1, T2, T3> originalMethodInternal;

    internal InstanceInterceptorActionT3(Interceptor interceptor, Action<TTarget, T1, T2, T3> originalMethodInternal)
    {
    ...
    public DetourScope Intercept(TTarget instance, Action<T1, T2, T3> replacement)
    {
        ...
        var actual = (TTarget self, T1 t1, T2 t2, T3 t3) =>
        {
            if (typeof(TTarget).IsValueType ? EqualityComparer<TTarget>.Default.Equals(self, instance) : object.ReferenceEquals(self, instance))
                ...
                originalMethodInternal.Invoke(self, t1, t2, t3);
        };
...
public static partial class InstanceInterceptorExtensions
{
    ...
    public static InstanceInterceptorActionT3<TTarget, T1, T2, T3> InstanceJitest<TTarget, T1, T2, T3>(this TTarget instance, string methodName, out Action<T1, T2, T3> originalMethod)
    {
        ...
        var interceptor = instance.Jitest<Action<TTarget, T1, T2, T3>>(methodName, out var originalMethodInternal);
        originalMethod = (t1, t2, t3) => originalMethodInternal.Invoke(instance, t1, t2, t3);
        return new InstanceInterceptorActionT3<TTarget, T1, T2, T3>(interceptor, originalMethodInternal);
    }
}
