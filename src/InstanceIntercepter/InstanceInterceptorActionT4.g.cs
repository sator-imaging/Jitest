...
public sealed class InstanceInterceptorActionT4<TTarget, T1, T2, T3, T4>
{
    ...
    public DetourScope Intercept(TTarget instance, Action<T1, T2, T3, T4> replacement)
    {
        ...
        {
            if (typeof(TTarget).IsValueType ? EqualityComparer<TTarget>.Default.Equals(self, instance) : object.ReferenceEquals(self, instance)) replacement.Invoke(t1, t2, t3, t4);
            else originalMethod.Invoke(t1, t2, t3, t4);
        };
