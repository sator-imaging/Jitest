...
public sealed class InstanceInterceptor<TTarget, T1, T2>
{
...
public static class InstanceInterceptorExtensions_T2
{
    /// <summary>Extension method for instance method interceptor.</summary>
    public static InstanceInterceptor<TTarget, T1, T2> InstanceJitest<TTarget, T1, T2>(this TTarget instance, string methodName, out Action<T1, T2>? originalMethod)
    {
        ...
        if (typeof(TTarget).IsValueType)
        {
            ...
            originalMethod = null;
            return new InstanceInterceptor<TTarget, T1, T2>(method, null);
        }
        else
        {
            var (method, dele) = ResolveMethodHelper.ResolveMethod<Action<T1, T2>>(typeof(TTarget), methodName);
            originalMethod = dele;
            return new InstanceInterceptor<TTarget, T1, T2>(method, dele);
        }
    }
    /// <summary>Extension method for instance method interceptor.</summary>
    public static InstanceInterceptor<TTarget, T1, T2> InstanceJitest<TTarget, T1, T2>(this TTarget instance, string methodName, out Func<T1, T2>? originalMethod)
    {
        if (instance == null) throw new ArgumentNullException(nameof(instance));
        if (typeof(TTarget).IsValueType)
        {
        ...
        {
            var (method, dele) = ResolveMethodHelper.ResolveMethod<Func<T1, T2>>(typeof(TTarget), methodName);
            originalMethod = dele;
            return new InstanceInterceptor<TTarget, T1, T2>(method, dele);
        }
    }
