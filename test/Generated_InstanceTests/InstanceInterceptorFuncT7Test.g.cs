...
public class InstanceInterceptorFuncT7Test
{
    ...
    public async Task TestFunc7()
    {
        ...
        var targetOriginal = target.Func7(1, 2, 3, 4, 5, 6);
        var otherOriginal = other.Func7(1, 2, 3, 4, 5, 6);
        var interceptor = target.InstanceJitest<InstanceTestTargets, int, int, int, int, int, int, int>("Func7", out Func<int, int, int, int, int, int, int> originalMethod);

        // 1. Intercept specific instance
        using (interceptor.Intercept(target, (t1, t2, t3, t4, t5, t6) => 8888))
        {
            var targetRes = target.Func7(1, 2, 3, 4, 5, 6);
            await Assert.That(targetRes).IsEqualTo(8888);

            var otherRes = other.Func7(1, 2, 3, 4, 5, 6);
        ...
        var afterRes = target.Func7(1, 2, 3, 4, 5, 6);
