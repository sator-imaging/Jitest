...
{
    [Test]
    public async Task TestGetInt_10()
    {
        var target = new InstanceTestTargets();
        var other = new InstanceTestTargets();
        var targetOriginal = target.GetInt_10(1, 2, 3, 4, 5, 6, 7, 8, 9);
        var otherOriginal = other.GetInt_10(1, 2, 3, 4, 5, 6, 7, 8, 9);
        var interceptor = target.InstanceJitest<InstanceTestTargets, int, int, int, int, int, int, int, int, int, int>("GetInt_10", out Func<int, int, int, int, int, int, int, int, int, int> originalMethod);

        // 1. Intercept specific instance
        using (interceptor.Intercept(target, (t1, t2, t3, t4, t5, t6, t7, t8, t9) => 8888))
        {
            var targetRes = target.GetInt_10(1, 2, 3, 4, 5, 6, 7, 8, 9);
            await Assert.That(targetRes).IsEqualTo(8888);

            var otherRes = other.GetInt_10(1, 2, 3, 4, 5, 6, 7, 8, 9);
        ...
        var afterRes = target.GetInt_10(1, 2, 3, 4, 5, 6, 7, 8, 9);
