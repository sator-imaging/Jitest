...
{
    [Test]
    public async Task TestFunc2()
    {
        var target = new InstanceTestTargets();
        var other = new InstanceTestTargets();
        var targetOriginal = target.Func2(1);
        var otherOriginal = other.Func2(1);
        var interceptor = target.InstanceJitest<InstanceTestTargets, int, int>("Func2", out Func<int, int> originalMethod);

        // 1. Intercept specific instance
        using (interceptor.Intercept(target, (t1) => 8888))
        {
            var targetRes = target.Func2(1);
            await Assert.That(targetRes).IsEqualTo(8888);

            var otherRes = other.Func2(1);
