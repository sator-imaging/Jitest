...
{
    [Test]
    public async Task TestGetInt_1()
    {
        var target = new InstanceTestTargets();
        var other = new InstanceTestTargets();
        var targetOriginal = target.GetInt_1();
        var otherOriginal = other.GetInt_1();
        var interceptor = target.InstanceJitest<InstanceTestTargets, int>("GetInt_1", out Func<int> originalMethod);

        // 1. Intercept specific instance
        using (interceptor.Intercept(target, () => 8888))
        {
            var targetRes = target.GetInt_1();
            await Assert.That(targetRes).IsEqualTo(8888);

            var otherRes = other.GetInt_1();
