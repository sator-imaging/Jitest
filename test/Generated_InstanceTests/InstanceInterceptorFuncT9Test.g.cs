...
{
    [Test]
    public async Task TestGetInt_9()
    {
        var target = new InstanceTestTargets();
        var other = new InstanceTestTargets();
        var targetOriginal = target.GetInt_9(1, 2, 3, 4, 5, 6, 7, 8);
        var otherOriginal = other.GetInt_9(1, 2, 3, 4, 5, 6, 7, 8);
        var interceptor = target.InstanceJitest<InstanceTestTargets, int, int, int, int, int, int, int, int, int>("GetInt_9", out Func<int, int, int, int, int, int, int, int, int> originalMethod);

        // 1. Intercept specific instance
        using (interceptor.Intercept(target, (t1, t2, t3, t4, t5, t6, t7, t8) => 8888))
        {
            var targetRes = target.GetInt_9(1, 2, 3, 4, 5, 6, 7, 8);
            await Assert.That(targetRes).IsEqualTo(8888);

            var otherRes = other.GetInt_9(1, 2, 3, 4, 5, 6, 7, 8);
        ...
        var afterRes = target.GetInt_9(1, 2, 3, 4, 5, 6, 7, 8);
