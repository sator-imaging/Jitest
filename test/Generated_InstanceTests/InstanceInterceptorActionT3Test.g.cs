...
public class InstanceInterceptorActionT3Test
{
    ...
    public async Task TestVoid_3()
    {
        ...
        using (interceptor.Intercept(target, (t1, t2, t3) => { targetIntercepted = true; }))
        {
            ...
            await Assert.That(target.Void_3Count).IsEqualTo(0);

            other.Void_3(1, 2, 3);
            await Assert.That(other.Void_3Count).IsGreaterThan(0);
        }
        ...
        int targetCountBefore = target.Void_3Count;
        originalMethod.Invoke(1, 2, 3);
        await Assert.That(target.Void_3Count).IsGreaterThan(targetCountBefore);

        // 4. Reverted after disposal
        int otherCountBefore = other.Void_3Count;
        other.Void_3(1, 2, 3);
        await Assert.That(other.Void_3Count).IsGreaterThan(otherCountBefore);
    }
}
